...
fn encode_json_simd(value: &serde_json::Value) -> Result<String, Box<dyn Error>> {
...
fn parse_json_simd(value: &mut str) -> Result<serde_json::Value, Box<dyn Error>> {
    let result = simd_json::from_str(value)?;
    Ok(result)
}
...
fn criterion_benchmark_2(c: &mut Criterion) {
    ...
    let bytes = simple_dataset.as_bytes();
    let json = serde_json::from_str(simple_dataset.as_str()).unwrap();
    let mut group = c.benchmark_group("encode_json_simd");
    group
        .sample_size(500)
        // .warm_up_time(Duration::from_secs(3))
        .throughput(Throughput::Bytes(bytes.len() as u64));
    group.bench_function("encode_json", |b| b.iter(|| encode_json_simd(&json)));
    group.finish();
}
...
fn criterion_benchmark_3(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("parse_json_simd");
    group
        .sample_size(500)
        // .warm_up_time(Duration::from_secs(3))
        .throughput(Throughput::Bytes(bytes.len() as u64));
    group.bench_function("parse_json", |b| {
        b.iter(|| parse_json_simd(twitter_dataset.as_mut_str()))
    });
    group.finish();
}
